...
RegisterNetEvent('mnc-engineswap:openAdminMenu', function()
    -- Build top-level category menu
    local categoryOptions = {}
 
    for _, category in ipairs(Config.EngineSounds) do
        table.insert(categoryOptions, {
            title = category.category,
            description = #category.engines .. ' engines available',
            icon = 'wrench',
            arrow = true,
            onSelect = function()
                -- Build engine list for this category
                local engineOptions = {}

                for _, engine in ipairs(category.engines) do
                    table.insert(engineOptions, {
                        title = engine.name,
                        description = engine.description,
                        icon = 'cog',
                        metadata = {
                            { label = 'Sound Hash', value = engine.sound },
                        },
                        onSelect = function()
                            local confirmed = lib.alertDialog({
                                header = 'Admin Engine Swap',
                                content = ('Apply **%s** engine to your current vehicle?\n\nThis is free and instant — no delivery or payment.'):format(engine.name),
                                centered = true,
                                cancel = true,
                                labels = { confirm = 'Apply Engine', cancel = 'Cancel' }
                            })

                            if confirmed ~= 'confirm' then
                                TriggerEvent('mnc-engineswap:openAdminMenu')
                                return
                            end
                            ...
                            if not vehicle or vehicle == 0 or not DoesEntityExist(vehicle) then
                                lib.notify({ title = 'Admin Engine Swap', description = 'You must be sitting in a vehicle.', type = 'error' })
                                return
                            end

                            local plate = GetVehicleNumberPlateText(vehicle)
                            plate = string.trim(plate)
                            plate = string.upper(plate)

                            if plate == "" or plate == " " then
                                lib.notify({ title = 'Error', description = 'Invalid vehicle plate.', type = 'error' })
                                return
                            end

                            -- === IMPROVED APPLICATION SEQUENCE ===
                            SetVehicleEngineOn(vehicle, true, false, true)
                            Wait(50)

                            -- Primary sound apply
                            local soundOk = pcall(function()
                                ForceVehicleEngineAudio(vehicle, engine.sound)
                            end)

                            Wait(100)

                            -- Secondary force (this helps with stubborn custom sounds)
                            pcall(function()
                                ForceVehicleEngineAudio(vehicle, engine.sound)
                            end)
                            ...
                            -- Apply handling
                            local handlingOk = false
                            local vehicleModel = GetHashKey(engine.sound)
                            if IsModelInCdimage(vehicleModel) and IsModelAVehicle(vehicleModel) then
                                local spawnPos = GetOffsetFromEntityInWorldCoords(ped, 0.0, -1000.0, -1000.0)
                                RequestModel(vehicleModel)
                                local timeout = 0
                                while not HasModelLoaded(vehicleModel) and timeout < 150 do
                                    Wait(10)
                                    timeout = timeout + 1
                                end
                                if HasModelLoaded(vehicleModel) then
                                    local tempVeh = CreateVehicle(vehicleModel, spawnPos.x, spawnPos.y, spawnPos.z, 0.0, false, false)
                                    if DoesEntityExist(tempVeh) then
                                        local df  = GetVehicleHandlingFloat(tempVeh, "CHandlingData", "fInitialDriveForce")
                                        local vel = GetVehicleHandlingFloat(tempVeh, "CHandlingData", "fInitialDriveMaxFlatVel")
                                        local tc  = GetVehicleHandlingFloat(tempVeh, "CHandlingData", "fTractionCurveMax")
                                        DeleteVehicle(tempVeh)
                                        SetModelAsNoLongerNeeded(vehicleModel)
                        
                                        if df > 0.0 and vel > 0.0 and tc > 0.0 then
                                            handlingOk = pcall(function()
                                                SetVehicleHandlingFloat(vehicle, "CHandlingData", "fInitialDriveForce", df)
                                                SetVehicleHandlingFloat(vehicle, "CHandlingData", "fInitialDriveMaxFlatVel", vel)
                                                SetVehicleHandlingFloat(vehicle, "CHandlingData", "fTractionCurveMax", tc)
                            ...
                            -- ALWAYS save
                            TriggerServerEvent('mnc-engineswap:saveEngine', plate, engine.sound)

                            lib.notify({
                                title = 'Admin Engine Swap',
                                description = ('**%s** engine applied & saved to plate **%s**'):format(engine.name, plate),
                                type = 'success'
                            })

                            if Config.Debug then
                                print(("^2[mnc-engineswap]^7 Admin applied '%s' (%s) to plate %s | soundOk=%s | handlingOk=%s"):format(
                                    engine.name, engine.sound, plate, tostring(soundOk), tostring(handlingOk)))
                            end
                        end
                    })
                end
                ...
                lib.registerContext({
                    id = 'admin_engineswap_engines',
                    title = '🔧 ' .. category.category .. ' Engines',
                    options = engineOptions
                })
                lib.showContext('admin_engineswap_engines')
            end
        })
    end

    lib.registerContext({
        id = 'admin_engineswap_categories',
        title = '🔧 Admin Engine Swap',
        options = categoryOptions
    })
    lib.showContext('admin_engineswap_categories')
end)
 
-- Allow the event to also be triggered locally (e.g. re-opening after back navigation)
RegisterNetEvent('mnc-engineswap:openAdminMenu')
AddEventHandler('mnc-engineswap:openAdminMenu', function()
    -- already registered above as a net event, this catches local TriggerEvent calls too
end)
...
-- =====================================================================
-- VEHICLE MODEL SOUND META — CLIENT
-- =====================================================================

-- Helper: build a flat list of all engine options from Config.EngineSounds
local function GetAllEngineOptions()
    local options = {}
    for _, category in ipairs(Config.EngineSounds) do
        for _, engine in ipairs(category.engines) do
            table.insert(options, {
                label = ('[%s] %s'):format(category.category, engine.name),
                sound = engine.sound,
                name  = engine.name
            })
        end
...
-- Open the engine picker for a given model, then save to DB
local function OpenEnginePicker(model, onPicked)
    local allEngines = GetAllEngineOptions()
    local categoryMap = {}
    local categoryOrder = {}

    -- Re-group by category for a two-level menu
    for _, category in ipairs(Config.EngineSounds) do
        if not categoryMap[category.category] then
            categoryMap[category.category] = {}
            table.insert(categoryOrder, category.category)
        end
        for _, engine in ipairs(category.engines) do
            table.insert(categoryMap[category.category], engine)
        end
    end

    -- Build top-level category options
    local catOptions = {}
    for _, catName in ipairs(categoryOrder) do
        local engines = categoryMap[catName]
        table.insert(catOptions, {
            title       = catName,
            description = #engines .. ' engine sounds',
            icon        = 'layer-group',
            arrow       = true,
            onSelect    = function()
                -- Build engine list for this category
                ...
                for _, engine in ipairs(engines) do
                    table.insert(engOptions, {
                        title       = engine.name,
                        description = engine.description,
                        icon        = 'volume-high',
                        metadata    = { { label = 'Sound Hash', value = engine.sound } },
                        onSelect    = function()
                            local confirmed = lib.alertDialog({
                                header  = 'Set Model Default Sound',
                                content = ('Set **%s** as the default engine sound for model **%s**?'):format(engine.name, model),
                                centered = true,
                                cancel  = true,
                                labels  = { confirm = 'Set Default', cancel = 'Cancel' }
                            })
                            if confirmed ~= 'confirm' then
                                -- Re-open from category level
                                TriggerEvent('mnc-engineswap:openVehSoundMetaMenu')
                                return
                            end
                            onPicked(engine)
                        end
                ...
                lib.registerContext({
                    id      = 'vehsoundmeta_engines',
                    title   = catName .. ' — ' .. model,
                    menu    = 'vehsoundmeta_categories',
                    options = engOptions
                })
                lib.showContext('vehsoundmeta_engines')
            end
        })
    end

    lib.registerContext({
        id      = 'vehsoundmeta_categories',
        title   = 'Select Sound — ' .. model,
        options = catOptions
    })
    lib.showContext('vehsoundmeta_categories')
end

-- Main admin meta menu
RegisterNetEvent('mnc-engineswap:openVehSoundMetaMenu', function()
    -- Detect the vehicle the admin is currently in (or closest)
    local ped        = PlayerPedId()
    local vehicle    = GetVehiclePedIsIn(ped, false)
    local modelName  = nil
    local currentSoundLabel = 'None set'

    if vehicle and vehicle ~= 0 and DoesEntityExist(vehicle) then
        local modelHash = GetEntityModel(vehicle)
        -- Convert hash back to a readable string via game pool lookup
        modelName = GetDisplayNameFromVehicleModel(modelHash)
        if not modelName or modelName == 'CARNOTFOUND' then
            modelName = tostring(modelHash) -- Fallback to raw hash string
        end
        modelName = string.lower(modelName)
    end

    -- Fetch all existing overrides to display in the manage section
    QBCore.Functions.TriggerCallback('mnc-engineswap:getAllModelSounds', function(rows)
        local options = {}

        -- Section 1: Set sound for current vehicle model
        if modelName then
            table.insert(options, {
                title       = '🚗 Set Sound for Current Model',
                description = 'Model: ' .. modelName,
                icon        = 'plus',
                arrow       = true,
                onSelect    = function()
                    OpenEnginePicker(modelName, function(engine)
                        TriggerServerEvent('mnc-engineswap:saveModelSound', modelName, engine.sound)
                        -- Immediately apply to this vehicle so admin can hear it
                        SetVehicleEngineOn(vehicle, true, false, true)
                        pcall(function() ForceVehicleEngineAudio(vehicle, engine.sound) end)
                    end)
                end
            })
        else
            table.insert(options, {
                title       = '⚠️ No Vehicle Detected',
                description = 'Enter a vehicle to set its model sound.',
                icon        = 'triangle-exclamation',
                disabled    = true
            })
        end

        -- Divider-style header for existing overrides
        table.insert(options, {
            title       = '📋 Existing Model Overrides (' .. #rows .. ')',
            description = 'Select an entry to remove it',
            icon        = 'database',
            disabled    = #rows == 0,
            -- Only act as a header when empty; otherwise entries below are actionable
        })

        -- Section 2: List all saved overrides with option to delete
        for _, row in ipairs(rows) do
            local rowModel = row.model
            local rowSound = row.engine_sound
            table.insert(options, {
                title       = rowModel,
                description = 'Sound: ' .. rowSound,
                icon        = 'trash-can',
                metadata    = { { label = 'Sound Hash', value = rowSound } },
                onSelect    = function()
                    local confirmed = lib.alertDialog({
                        header  = 'Remove Override',
                        content = ('Remove the default sound override for model **%s**?'):format(rowModel),
                        centered = true,
                        cancel  = true,
                        labels  = { confirm = 'Remove', cancel = 'Keep' }
                    })
                    if confirmed == 'confirm' then
                        TriggerServerEvent('mnc-engineswap:deleteModelSound', rowModel)
                    end
                    -- Re-open menu to reflect change
                    Wait(300)
                    TriggerEvent('mnc-engineswap:openVehSoundMetaMenu')
                end
            })
        ...
        lib.registerContext({
            id      = 'vehsoundmeta_main',
            title   = '🔧 Vehicle Model Sound Meta',
            options = options
        })
        lib.showContext('vehsoundmeta_main')
    end)
end)

-- =====================================================================
-- MODEL SOUND FALLBACK — apply model-level default on vehicle enter
-- (only fires if the plate has no individual engine swap saved)
-- =====================================================================
CreateThread(function()
    local lastMetaPlate = nil
    while true do
        local ped     = PlayerPedId()
        local vehicle = GetVehiclePedIsIn(ped, false)
        ...
        if vehicle ~= 0 and DoesEntityExist(vehicle) and GetPedInVehicleSeat(vehicle, -1) == ped then
            local plate = GetVehicleNumberPlateText(vehicle)
            if plate and plate ~= lastMetaPlate then
                lastMetaPlate = plate

                -- Only apply the model fallback if no per-plate swap exists
                QBCore.Functions.TriggerCallback('mnc-engineswap:getEngine', function(savedSound)
                    if not savedSound then
                        -- No plate-level swap — check for a model-level default
                        local modelHash = GetEntityModel(vehicle)
                        local modelName = string.lower(GetDisplayNameFromVehicleModel(modelHash) or '')
                        if modelName ~= '' and modelName ~= 'carnotfound' then
                            QBCore.Functions.TriggerCallback('mnc-engineswap:getModelSound', function(modelSound)
                                if modelSound and DoesEntityExist(vehicle) then
                                    SetVehicleEngineOn(vehicle, true, false, true)
                                    pcall(function() ForceVehicleEngineAudio(vehicle, modelSound) end)
                                    if Config.Debug then
                                        print(("^2[mnc-engineswap]^7 Applied model-meta sound '%s' to model '%s' (plate: %s)"):format(modelSound, modelName, plate))
                                    end
                                end
                            end, modelName)
                        end
